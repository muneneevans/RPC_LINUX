...
static void
divide_prog_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
		pair divide_1_arg;
	} argument;
...
main (int argc, char **argv)
{
	...
	pmap_unset (DIVIDE_PROG, DIVIDE_VERS);
	...
	if (!svc_register(transp, DIVIDE_PROG, DIVIDE_VERS, divide_prog_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (DIVIDE_PROG, DIVIDE_VERS, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, DIVIDE_PROG, DIVIDE_VERS, divide_prog_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (DIVIDE_PROG, DIVIDE_VERS, tcp).");
		exit(1);
	}
